...
case class TableMachwell(name: String) {
    ...
    val columnsMap: Map[String, String] = table.columns.map(col => (col.name, col.typeName)).toMap
    ...
    def insert(data: JsValue) = {
        ...
        println(cmd)
        val command = SQL(cmd)
        val casted = object_.map {
            x => castValue(x._1, x._2)
        }.toList
...
object Run extends App {
    ...
    val json = Json.parse("""{"age": 42, "name": "Michele"}""") 
